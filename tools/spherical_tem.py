...
# Plotting (interactive for click-threshold)
...
def try_read_dm3(file_bytes: bytes) -> DM3Image:
    """
    Read a .dm3 using ncempy and try to infer pixel size (nm/px).
    If scale is missing, we return nm_per_px = np.nan and the UI will ask for it.
    """
    if ncem_dm is None:
        raise RuntimeError(
            "ncempy is not installed. Please install it (pip install ncempy)."
        )
    ...
    # Try to locate pixel size from metadata in meters (common), fallback: nan
    nm_per_px = np.nan
    md = rdr.allTags
    # Common places people store pixel size in dm3; we try a few keys defensively
    candidates = [
...
def gmm_threshold(data: np.ndarray, nbins: Optional[int] = None, sample=200_000) -> float:
    ...
    # Fit 2-component GMM on intensities
    gm = GaussianMixture(n_components=2, random_state=42)
    gm.fit(flat.reshape(-1, 1))
    mu = np.sort(gm.means_.flatten())
    left_mu, right_mu = mu[0], mu[1]

    # Find histogram valley between the two means (similar to MATLAB approach)
    centers, counts = histogram_for_intensity(flat, nbins)
    in_range = (centers >= left_mu) & (centers <= right_mu)
    if not np.any(in_range):
        # Fallback to midpoint if histogram is degenerate
        return float((left_mu + right_mu) / 2)
    sub_centers = centers[in_range]
    sub_counts = counts[in_range]
    thr = sub_centers[np.argmin(sub_counts)]
    return float(thr)


def segment_and_measure(data: np.ndarray, threshold: float, nm_per_px: float,
                        min_area_px: int = 5, circ_lo: float = 0.05, circ_hi: float = 3.0):
    """
    Segments nanoparticles: simple threshold + watershed mask,
    then measures mean of major/minor axes (diameter proxy), circularity,
    filters by circularity like MATLAB, and converts to nm.
    """
    # Binary (note MATLAB uses im_Green < threshold)
    im_bi = data < threshold

    # Simple cleanup
    im_bi = remove_small_objects(im_bi, min_size=min_area_px)

    # Watershed mask on inverted image (MATLAB used imcomplement)
    # A practical variant: distance transform on binary -> watershed on peaks
    dist = distance_transform_edt(im_bi)
    # Invert distance for watershed basins
    ...
    centers = []
    for p in props:
        # major/minor axis length can be 0 if degenerate; guard it
        maj = getattr(p, "major_axis_length", 0.0) or 0.0
        minr = getattr(p, "minor_axis_length", 0.0) or 0.0
        if maj == 0 and minr == 0:
            continue
        diam_px.append(np.mean([maj, minr]))
        area = float(p.area)
        perim = float(p.perimeter) if getattr(p, "perimeter", 0.0) else 0.0
        if perim > 0:
    ...
    # Filter circularities like MATLAB
    keep = (circ >= circ_lo) & (circ <= circ_hi)
    diam_px = diam_px[keep]

    # Convert to nm
    diam_nm = diam_px * nm_per_px
    # Remove tiny diameters (like MATLAB post-filter <2 nm)
    diam_nm = diam_nm[diam_nm >= 2.0]
...
def fig_intensity_histogram(data: np.ndarray, nbins: Optional[int] = None, threshold: Optional[float] = None):
    ...
    if threshold is not None:
        fig.add_shape(
            type="line",
            x0=threshold, x1=threshold, y0=0, y1=max(counts) if len(counts) else 1,
            line=dict(width=2, dash="dash")
        )
    return fig


def save_hist_as_png(fig: go.Figure, filename_prefix: str) -> bytes:
    """
    Renders a Plotly figure to a PNG bytes object using kaleido.
    """
    # Requires kaleido
    return fig.to_image(format="png")  # type: ignore
